# FRONTEND
The frontend code is made up of simple HTML, CSS, javascript code necessary to create the content, web interface used by the end user can use to interact with the website.

This folder consists of build instructions involved in the CI pipeline to build the frontend component of the Website.

At the end of this section we'll prepare a helm chart and upload it to a github repository [_here_]() where all the sites components helm-charts exist, that will deploy the necessary components to start up the frontend for our website.

## BUILD INSTRUCTION
---

The following steps are to be performed to build the  frontend component
...
- **CREATE CONTAINER IMAGE**
    ...
    4. **STEP-4** - Setup nexus repository [_refer_](../../CD), and then configure a new docker repo by following the below mentioned steps 
        ...
        -  Now **Enter a repository name > Enter a preffered HTTP port > Enable Docker API > Create Repository**
    ...
    2. **STEP-5** - Configure nexus user credentials in jekins. 
    ...
    2. **STEP-6** - Setup the downstream job necessary to update the helm chart files [ [**_here_**](https://github.com/sai-harsha-dev/DevOps_Project_HelmChart.git) ] with updated Image tag to the manifest files. Do the following steps in jenkins console.
        ...
        ```console
           sed -i '/^image:.*$/d' templates/frontend/values.yaml
           echo "image: ${IMAGE}" >> templates/frontend/values.yaml 
           git add templates/frontend/values.yaml
    ...
    2. **STEP-7** - Go to the jenkins upstream-job ( _**Refer STEP 3**_ ) and perform following steps to build frontend image & Trigger downstream job to update image tag in helm charts 
